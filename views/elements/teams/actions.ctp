...
if (Configure::read('feature.attendance') && $team['track_attendance']) {
	...
	if ($this->params['controller'] != 'teams' || $this->params['action'] != 'attendance') {
		if ($is_admin || in_array($team['id'], $this->UserCache->read('AllTeamIDs'))) {
...
if ($is_admin || $is_manager) {
	...
	$url = array('controller' => 'teams', 'action' => 'delete', 'team' => $team['id']);
...
if ($is_logged_in && Configure::read('feature.annotations')) {
	...
	$links[] = $this->Html->link($link, array('controller' => 'teams', 'action' => 'note', 'team' => $team['id'], 'return' => true));
}
