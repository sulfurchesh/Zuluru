...
class TeamsController extends AppController {
	...
	function emails() {
		...
		$this->Team->contain(array (
			'Person' => array(
				...
				'conditions' => array(
					'Person.id !=' => $this->UserCache->currentId(),
	...
	function add_player() {
		...
		$teams = $this->Team->Person->read(null, $this->UserCache->currentId());
	...
	function roster_role() {
		...
		if (in_array($role, $required_roles) &&
			!in_array($this->data['Person']['role'], $required_roles) &&
			!$this->is_admin && !$this->is_manager)
		{
			$count = 0;
			foreach (Configure::read('required_roster_roles') as $captain_role) {
